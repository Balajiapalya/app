...
const VideoList = ({ i, create_On, created_time }) => {
    ...
    return (
        <>
            ...
            {i.thumbnailUrl ? <td className={`${styles.thumbnail} ${styles.tdCursor}`}  onClick={() => handleChange(i.status)}><img width="100px" height="70px" src={`${i.thumbnailUrl}`} alt="image"></img></td> : <td className={`${styles.thumbnail} ${styles.tdCursor}`} onClick={() => handleChange(i.status)}><img src='/images/placeholder.png' width="100px" /></td>}
